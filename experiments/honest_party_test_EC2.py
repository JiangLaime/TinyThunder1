...
import gevent
import os
from misc.utils import ACSException, checkExceptionPerGreenlet, encodeTransaction, getKeys, \
...
def connect_to_channel(hostname, port, party):
    ...
    def _handle():
        while True:
            ...
            try:
                # < 表示使用小端序的方式进行打包
                # I 表示使用 4 个字节（32 位）来表示一个整数，这种方式能够确保数据在不同系统之间的传输时能够被正确解析。
...
def client_test_freenet(N, t, options):
    ...
    def makeBroadcast(i):
        ...
        for j in range(N):
            ...
            # connect_to_channel返回一个Queue（），为每个Queue()创建一个协程，接受Queue中的数据，并以特定格式发送给建立连接的主机。
    ...
    if True:  # We only test for once
        ...
        def _makeBroadcast(x):
            broadcast, send = makeBroadcast(x)
            bcList[x] = broadcast
            sdList[x] = send
        ...
        print (indexID, "waits for", time_now + delay, 'now is', time_now)
        s.run()
