...
nameList = open(os.path.dirname(os.path.abspath(__file__)) + '\\..\\experiments\\names.txt','r').read().strip().split('\n')
...
def deepEncode(mc, m):
    ...
    if bundle[0] == 'O':
        ...
        # 这里的 tag 统一都是 'B' ，见includeTransaction Line 211.
        (tag, c) = bundle
...
def initiateRND(TX):
    ...
    # LONG_RND_STRING取 1e6 和 TX * (TR_SIZE-5)的最小值，TR_SIZE=250
