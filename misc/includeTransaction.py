...
def multiSigBr(pid, N, t, msg, broadcast, receive, outputs, send):
    ...
    def Listener():
        ...
        while True:  # main loop
            ...
            elif msgBundle[0] == 'e': # ECHO message
                if keys[sender].verify(sha1hash(''.join([str(msgBundle[1][0]), msgBundle[1][1], msgBundle[1][2], ''.join(msgBundle[1][3])])), msgBundle[2]):
                    # msgBundle[1] = newBundle
                    # newBundle = (sender, msgBundle[1][0], msgBundle[1][1], msgBundle[1][2])
                                           # msgBundle[1] = (encodedFragList[i], rootHash, mb)
                    originBundle = msgBundle[1]
                    # originBundle = (sender, msgBundle[1][0], msgBundle[1][1], msgBundle[1][2])
                    ...
                    raise ECDSASignatureError()
            elif msgBundle[0] == 'r': # READY message
